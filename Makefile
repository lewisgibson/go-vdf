...
format: ## Format files
	...
	if command -v custom-gcl >/dev/null 2>&1; then \
		...
		go run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.11.4 fmt -c "$$cfg" ./...; \
	fi
...
lint: ## Lint files
	...
	if command -v custom-gcl >/dev/null 2>&1; then \
		...
		go run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.11.4 run --fix -c "$$cfg" ./...; \
	fi
